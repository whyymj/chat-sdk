...
## 进行中的 change
- `2026-07-30-refactor-module-extraction/`:模块抽离重构(可维护性)—— ① 纯函数抽离(`dataOps.ts` 18 个纯函数 → `tools/jsonUtils.ts` + 6 个 schema 工具 → `tools/schemaUtils.ts`;`useContextManager.ts` 6 个纯函数 → `composables/contextIndex.ts`);② 高频改动点抽离(`createChatSdk.ts` → `sdk/promptBuilder.ts` / `sdk/llmResolver.ts` / `sdk/conflictManager.ts` / `sdk/skillStore.ts` 桥接层);③ 对外开放 subpath(`./storage` / `./query` / `./llm`)。纯重构,运行时行为零变化;顶层 `.` 入口导出不变。分期:期一(P0 纯函数 + subpath,可独立发布)→ 期二(P1 状态机/桥接层)→ 期三(P2 低频可选)→ 期四(测试同步)→ 期五(文档 + 门禁 + 归档)。**【期一+期二已完成 2026-07-31】** 期一:jsonUtils(16)+schemaUtils(6)+promptBuilder 抽离 + ./storage/./query/./llm subpath;期二:contextIndex(6)+llmResolver(isChatModel/resolveLlm)+conflictManager(createConflictManager)抽离。createChatSdk 1751→1631、dataOps 969→670、useContextManager 321→235;sec-30/31/32 白盒单测,selftest 537→630 全过。skillStore 桥接评估延后(userSkills 被 12+ 处用、闭包时序交错,完整抽离风险 > 收益)。期三/四/五待实施。
