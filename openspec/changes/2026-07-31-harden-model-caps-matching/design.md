...
const fromTable = (() => {
  if (!opts.model) return undefined
  const hits = MODEL_TABLE.filter((e) => e.pattern.test(opts.model!))
  if (!hits.length) return undefined
  // longest-match:取 pattern 源字符串最长(最具体)的命中;与条目顺序无关
  hits.sort((a, b) => b.pattern.source.length - a.pattern.source.length)
  return hits[0].caps
})()
```

**为何 `pattern.source.length`**:正则源字符串长度是"具体度"的合理近似 —— `gpt-4o-mini`(11)长于 `gpt-4o`(6),`deepseek-v4`(10)长于 `deepseek`(8)。更长 pattern 通常含更多限定词,匹配范围更窄 = 更具体。
...
- **`source.length` 启发式偶有偏差**:两个 pattern 逻辑具体度与 source 长度不一致(罕见)。靠表驱动断言捕捉;真出现时该条目改 pattern(加边界 `(?<!-)` 等)而非依赖长度。
