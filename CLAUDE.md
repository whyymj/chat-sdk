...
#### 发布前必跑顺序
`npm run build` → `npm test`(364 全过) → `npm run test:e2e`(120 全过) → `npm pack --dry-run`(核对 files 不含 `.env`/`src`/`examples`/笔记) → 版本号递增 → `npm publish` → CDN 可达性验证(上节 5)
...
4. **构建+自测**:按「### 测试流程」末尾「发布前必跑顺序」执行(`npm run build` → `npm test` 364 全过 → `npm run test:e2e` 120 全过 → `npm pack --dry-run` 核对不含 `.env`/`src`/`examples`/笔记)
