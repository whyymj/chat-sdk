...
```bash
npm test            # tsx 跑 src/core/__tests__/selftest.ts(runner),1161 项断言
```
...
npm run test:e2e    # node 跑 tests/e2e-integration.mjs(runner),303 项断言
```
...
**按 demo 拆分**:测试代码在 `tests/browser/<demo>.spec.ts`(28 项断言):
...
- `session-history-demo.spec.ts`(3 项:新建会话→列表+1+当前切换 / 多会话切换点历史项→active 迁移 / 删除历史→列表-1+当前不可删;验证 listSessions/switchSession/deleteSession/sessionId 对外 API 浏览器端到端,session-history-management)
...
#### 发布前必跑顺序
`npm run build` → `npm test`(1161 全过) → `npm run test:e2e`(303 全过) → `npm run test:browser`(浏览器 E2E 全过) → `npm run test:exports`(types 与 src 导出对齐) → `npm run test:types`(tsconfig.test.json 只查对外 types/index.d.ts 类型对齐 + tests/types.test-d.ts;src 全量类型卫生用 `npx tsc -p tsconfig.json` 单独诊断,**非发布门禁** —— 勿把全量 tsc 报错当门禁阻塞) → `npm run test:size`(dist 体积不超阈值) → `npm pack --dry-run`(核对 files 不含 `.env`/`src`/`examples`/笔记) → 版本号递增 → `npm publish` → CDN 可达性验证(上节 5)
...
**计数同步**:补测试后同步更新本文件「测试流程」小节的断言计数(1161/303)与 README 中英文计数,以及下方测试矩阵的「改动范围」行(若引入新模块)。
...
4. **构建+自测**:按「### 测试流程」末尾「发布前必跑顺序」执行(`npm run build` → `npm test` 1161 全过 → `npm run test:e2e` 303 全过 → `npm run test:exports` 导出对齐 → `npm run test:types` 类型正确 → `npm run test:size` 体积不超阈值 → `npm pack --dry-run` 核对不含 `.env`/`src`/`examples`/笔记)
