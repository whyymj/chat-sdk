...
npm run test          # 自测(tsx 跑 src/__tests__/selftest.ts,992 项断言)
...
```bash
npm test            # tsx 跑 src/core/__tests__/selftest.ts(runner),992 项断言
```
...
#### 发布前必跑顺序
`npm run build` → `npm test`(992 全过) → `npm run test:e2e`(256 全过) → `npm run test:browser`(浏览器 E2E 全过) → `npm run test:exports`(types 与 src 导出对齐) → `npm run test:types`(tsc --noEmit 类型正确) → `npm run test:size`(dist 体积不超阈值) → `npm pack --dry-run`(核对 files 不含 `.env`/`src`/`examples`/笔记) → 版本号递增 → `npm publish` → CDN 可达性验证(上节 5)
...
**计数同步**:补测试后同步更新本文件「测试流程」小节的断言计数(992/254)与 README 中英文计数,以及下方测试矩阵的「改动范围」行(若引入新模块)。
...
4. **构建+自测**:按「### 测试流程」末尾「发布前必跑顺序」执行(`npm run build` → `npm test` 992 全过 → `npm run test:e2e` 256 全过 → `npm run test:exports` 导出对齐 → `npm run test:types` 类型正确 → `npm run test:size` 体积不超阈值 → `npm pack --dry-run` 核对不含 `.env`/`src`/`examples`/笔记)
