 ...
 */
export function createUsageHintsMiddleware(caps: HintCapabilityFlags | undefined, hasDataSlotOps: boolean, toolMode: 'simple' | 'advanced' | 'minimal' = 'simple'): Middleware {
  ...
  return {
    ...
    augmentPrompt: () => {
      ...
      if (hasDataSlotOps) {
        if (simple) {
          hints.push('读写数据槽用 read/write(高层入口,自动乐观锁 + 自动快照)。read({path}) 读当前值(返回含 hash,write 时自动比对,无需手动传);read() 不传 path 列出所有可操作槽。write({path, value}) 整体替换(value 直传 JSON 对象,如 {title:"x"},无需 stringify);write({path, value, patch:{op,jsonPath}}) 增量 patch(op=set/remove/merge/append);write({path, del:true}) 删除。写入自动经 schema 校验(失败不写)+ 自动存快照(出错可用 restore_data_snapshot 回退)。')
          hints.push('修改大对象/数组优先用 write 的 patch 增量(只发改动部分),避免整体重传被 max_tokens 截断致 JSON 不完整。')
          hints.push('在大数组里按条件筛选用 query_data_slot(JSONPath,如 $[?(@.type=="card" && @.price<100)]),返回匹配元素 path/index;定位后用 write patch 改。找名字记不清的元素用 search_data_slot(substring/regex/fuzzy)。批量过滤/映射/聚合/重写大数组用 eval_script(沙箱脚本,mode=query 只读/transform 落地)。')
        } else {
      ...
      if (caps?.subagents?.length) {
        ...
        hints.push(simple
          ...
          : '  · 方案定稿后,由你(主 agent)用 edit_data_slot 落地成 JSON(低温度执行 + schema 校验 + 写前确认)。')
