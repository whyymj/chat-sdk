...
export function createSdkEvents(onEvent?: SdkEventHandler): SdkEvents {
  ...
  const emit: SdkEventHandler = (event) => {
    ...
    if (onEvent) { try { onEvent(event) } catch (e) { console.warn('[page-agent-sdk] onEvent 回调出错(observable,已忽略):', asAgentError(e, 'observable').message) } }
    for (const l of listeners) { try { l(event) } catch (e) { console.warn('[page-agent-sdk] 事件监听器出错(observable,已忽略):', asAgentError(e, 'observable').message) } }
  }
