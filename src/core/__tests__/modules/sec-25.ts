...
export async function run(ctx: TestCtx): Promise<void> {
  ...
  assert(detectGarbledToolCall('<invoke name="set_data_slot">') === true, 'invoke name tag detected')
  ...
  assert(detectGarbledToolCall('please use set_data_slot to update') === false, 'normal mention of tool name not garbled')
}
